...
# google-ads SDK імпортується всередині функцій, які ходять у мережу (build_client,
# mutate). На верхньому рівні він робив `--self-test` незапускним там, де SDK не
# встановлено — тобто рівно на машині, де перевіряють логіку перед пушем.
CUSTOMER_ID = "9087037980"
LOGIN_CUSTOMER_ID = "1770392909"

# ad_group.id -> посадкова цієї групи. Правило одне: один намір — один документ.
#
# Історія цього рядка. До 25.08 всі три групи вели на головну — саме та «одна URL на
# три наміри», через яку post_click_quality_score був BELOW_AVERAGE скрізь. 01.09
# /uk/websites стала explainer-сторінкою (без цін і FAQ) і випала з ролі посадкової,
# тому «Сайт під ключ» тимчасово перевели на /uk/price — і хвороба повернулась у
# меншому масштабі: дві групи з різними намірами знову ділили один документ.
#
# 07.09 «Сайт під ключ» повернуто на головну, і це НЕ відкат до стану «все на /uk»:
#   • H1 головної — «Розробка сайтів та інтернет-магазинів під ключ», тобто дослівний
#     збіг із запитом групи; /uk/price відповідає на інше питання («скільки коштує»)
#     і для «сайт під ключ» є гіршою відповіддю, ніж головна;
#   • головна після етапів 1-2 CRO веде до заявки (три формати, прайс, форма) —
#     у серпні цього не було, і саме тому вона тоді не працювала як посадкова;
#   • головна більше не приймає ЖОДНОЇ іншої групи, тож перекриття немає.
# Підсумок: три групи — три різні документи, кожен відповідає на своє питання.
URL_BY_AD_GROUP = {
    "199656753776": "https://neuronics.work/uk",               # Сайт під ключ
...
def self_test():
    ...
    # Головна вимога до мапи: скільки груп — стільки різних документів. Саме її
    # порушення (три групи на одній URL) і дало BELOW_AVERAGE у 10 ключів із 10,
    # тому перевірка на унікальність тут строга, а не «мінімум дві».
    assert len(set(URL_BY_AD_GROUP.values())) == len(URL_BY_AD_GROUP) == 3, \
        "кожна група — своя посадкова; спільний URL повертає ту саму хворобу"
    # Префікс без слеша навмисно: з 07.09 одна з груп веде на сам /uk, і перевірка
    # з "…/uk/" мовчки перестала б покривати саме той рядок, який змінили.
    for u in URL_BY_AD_GROUP.values():
        assert u == "https://neuronics.work/uk" or u.startswith("https://neuronics.work/uk/"), \
            "тільки прод-домен і українська локаль: *.vercel.app віддає 401 і ловить відхилення"
        assert "#" not in u, "без якоря: якір не створює окремої сторінки для Google"
    print("self-test: 7 перевірок пройдено")
