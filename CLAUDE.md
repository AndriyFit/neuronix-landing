...
### Українська живе на `/`, англійська — на `/en`
`localePrefix: 'as-needed'` у `src/i18n/routing.ts`. До 2026-08-08 було `'always'`, і корінь
`neuronics.work/` назавжди лишався редіректом на `/uk` — Search Console рапортував
«Сторінка з переспрямуванням», а брендова адреса не мала шансу потрапити в індекс.

Три наслідки, які легко зламати:

- **`localeDetection: false` — не вмикати назад.** За замовчуванням next-intl визначає локаль
  з `Accept-Language`; Googlebot часто краулить з `en`, і корінь редіректив би вже на `/en`.
  Локаль перемикається лише явно, через `LanguageSwitcher`.
- **Постійні редіректи зі старих URL живуть у `next.config.ts`, не в middleware.** next-intl
  на префіксований шлях дефолтної локалі віддає 307 (тимчасовий), який не консолідує сигнали
  на новий URL. Редіректи з `next.config.ts` виконуються до middleware, тому там 308.
- **Абсолютні URL будуй тільки через `localeUrl()` з `src/lib/site.ts`** (`localeUrl('uk','/blog')`
  → `https://neuronics.work/blog`). Хардкод `${SITE_URL}/${locale}` раніше лежав у шести
  файлах — саме тому переїзд і зачепив стільки місць.

Матчер middleware мусить лишатися широким (`/((?!api|_next|_vercel|.*\..*).*)`) — безпрефіксні
шляхи `/blog`, `/privacy-policy` інакше не потраплять у next-intl.

### Блог існує ЛИШЕ українською
`getPostBySlug(slug)` локаль ігнорує — статті лежать в одному екземплярі. Тому `/en/blog/*`
віддавав той самий український текст із власним canonical: чистий дубль для Google плюс
брехливий `<html lang="en">`. Виправлено 2026-08-05: 308-редірект у `next.config.ts`,
`generateStaticParams` блогу повертає лише `uk`, canonical жорстко на україномовний URL,
hreflang прибрано, у sitemap лише україномовні URL. Після переїзду на `/` (08.08) цільові
шляхи безпрефіксні: `/en/blog/*` → `/blog/*`.

**Посилання на блог — звичайний `next/link` на літеральний `/blog`, не `Link` з
`@/i18n/navigation`.** Той підставив би поточну локаль (`/en/blog`), а з явним `locale="uk"` —
навпаки, завжди префікс (`/uk/blog`). Обидва варіанти дають 308 на кожному кліку й префетчі.
Для решти внутрішніх посилань (`/privacy-policy`) навпаки потрібен `Link` з `@/i18n/navigation`,
бо ці сторінки існують у двох мовах.

**Зʼявляться англійські статті — перш ніж вертати `/en/blog`, зроби, щоб `getPostBySlug`
приймав локаль.** Інакше дубль повернеться.
...
### `x-default` у hreflang — обов'язковий
Без нього Google складав `/en` і головну в одну групу й лишав `/en` поза індексом як
«альтернативну сторінку з належним тегом канонічної». `x-default` вказує, яку версію
показувати всім, чия мова не збіглася, — і розводить сторінки. Набір із трьох
(`uk`, `en`, `x-default`) збирається в `src/lib/metadata.ts`.
